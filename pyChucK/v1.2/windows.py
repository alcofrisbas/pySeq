...
class TimeCanvas(Canvas):
	def __init__(self, master, arr, arrG, **kwargs):
		...
		self.bind("<MouseWheel>", self.pitch)
	...
	def pitch(self,event):
		...
		self.create_rectangle(slot*self.WIDTH/lArr+self.WIDTH/lArr/3, self.HEIGHT/2-self.pitch+3, slot*self.WIDTH/lArr+self.WIDTH/lArr/3*2, self.HEIGHT/2-self.pitch-3, tags=("{}pitchLabel".format(str(slot))), fill = "dodgerblue3")
