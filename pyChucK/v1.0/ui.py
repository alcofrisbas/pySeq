...
- preferences for samples eg vol, rate

- make cues appear at the right intervals I THINK
- make cues take gains NO
- add tags to cues  DONE

- set gains by click in time canvas
"""
...
class TimeCanvas(Canvas):
	def __init__(self, master, **kwargs):
		...
		self.labelSubs.set(int(16))
		self.setupSubs()

	def toggle(self, event):
		slot =  event.x/(self.WIDTH/int(self.labelSubs.get()))
		if self.arr[slot] == 0:
			self.arr[slot] = 1
			self.create_rectangle(slot*self.WIDTH/int(self.labelSubs.get())+1, self.HEIGHT/8, slot*self.WIDTH/int(self.labelSubs.get())+self.WIDTH/int(self.labelSubs.get()), self.HEIGHT, fill = "dodgerblue", width=0)

		elif self.arr[slot] == 1:
			self.arr[slot] = 0
			self.create_rectangle(slot*self.WIDTH/int(self.labelSubs.get())+1, self.HEIGHT/8, slot*self.WIDTH/int(self.labelSubs.get())+self.WIDTH/int(self.labelSubs.get()), self.HEIGHT, fill = "white", width=0)
		#print self.arr
		self.create_text(slot*self.WIDTH/int(self.labelSubs.get())+self.WIDTH/int(self.labelSubs.get())/2, self.HEIGHT-3, text = str(slot), anchor = S, font="Symbol 8")

	def setupSubs(self):
		self.delete("all")
		self.arr = []
		for i in range(int(self.labelSubs.get())):
			self.arr.append(0)
			self.create_line(i*self.WIDTH/int(self.labelSubs.get()), -0, i*self.WIDTH/int(self.labelSubs.get()), self.HEIGHT)
			self.create_text(i*self.WIDTH/int(self.labelSubs.get())+self.WIDTH/int(self.labelSubs.get())/2, self.HEIGHT-3, text = str(i), anchor = S, font="Symbol 8")
		#print self.arr


	def addSub(self):
		new = int(self.labelSubs.get()) + 1
...
class InstWindow(Toplevel):
	def __init__(self, master, can, originX, originY, instrumentDict, lineX, lineY, iName = "track_name"):
		self.can = can
	...
	# 	if self.arr[slot] == 0:
	# 		self.arr[slot] = 1
	# 		self.timeCan.create_rectangle(slot*self.WIDTH/int(self.labelSubs.get())+1, 0, slot*self.WIDTH/int(self.labelSubs.get())+self.WIDTH/int(self.labelSubs.get()), self.HEIGHT, fill = "blue", width=0)

	# 	elif self.arr[slot] == 1:
	# 		self.arr[slot] = 0
	# 		self.timeCan.create_rectangle(slot*self.WIDTH/int(self.labelSubs.get())+1, 0, slot*self.WIDTH/int(self.labelSubs.get())+self.WIDTH/int(self.labelSubs.get()), self.HEIGHT, fill = "white", width=0)
	# 	#print self.arr
	# 	self.timeCan.create_text(slot*self.WIDTH/int(self.labelSubs.get())+self.WIDTH/int(self.labelSubs.get())/2, self.HEIGHT-3, text = str(slot), anchor = S, font="Symbol 8")

	# def setupSubs(self):
	# 	self.timeCan.delete("all")
	# 	self.arr = []
	# 	for i in range(int(self.labelSubs.get())):
	# 		self.arr.append(0)
	# 		self.timeCan.create_line(i*self.WIDTH/int(self.labelSubs.get()), 0, i*self.WIDTH/int(self.labelSubs.get()), self.HEIGHT)
	# 		self.timeCan.create_text(i*self.WIDTH/int(self.labelSubs.get())+self.WIDTH/int(self.labelSubs.get())/2, self.HEIGHT-3, text = str(i), anchor = S, font="Symbol 8")
	# 	#print self.arr
	...
	def make(self):
		...
		i.setArray(self.timeCan.arr)
		b = Box(i, self.oX, self.oY)
		self.iDict[i.n] = b
...
class GUI:
	...
	def inst1(self, event):
		...
		InstWindow(self, self.can, inst.oX, inst.oY, self.instruments, 60, 60, iName = inst.instrument.n)
	...
	def mouse2(self, event):
		...
		if "current" not in tags:
			InstWindow(self, self.can, event.x, event.y, self.instruments, 60, 60)
